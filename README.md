...
- **Αποτροπή ανάμειξης δεδομένων ταινιών**: Η επιλογή TMDB αποτελέσματος κάνει πλέον ranking βάσει ομοιότητας τίτλου + έτους, και απορρίπτει matches με έτος > 2 χρόνια διαφορά και ομοιότητα τίτλου < 40%
- **Επαλήθευση αποθηκευμένου `tmdb_id`**: Πριν χρησιμοποιηθεί αποθηκευμένο `tmdb_id`, ελέγχεται αν ανήκει όντως στη σωστή ταινία — αν όχι, καθαρίζεται και γίνεται νέα αναζήτηση
- **Βελτιωμένα TMDB metadata**: Τα πεδία `title`, `original_title`, `year` πλέον συμπεριλαμβάνονται στο enrichment response για εσωτερική χρήση
